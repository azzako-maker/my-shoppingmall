...
- [x] 주문 내역 조회
  - [x] 마이페이지 (`/my-page`)
    - [x] 페이지 라우트 생성 (`app/my-page/page.tsx`)
    - [x] 인증 확인 (미로그인 시 리다이렉트)
    - [x] 로딩 스켈레톤 (`app/my-page/loading.tsx`)
  - [x] 주문 목록 조회 Server Action
    - [x] `getOrders()` 함수 이미 구현됨 (`actions/orders.ts`)
    - [x] `orders` 테이블에서 `clerk_id`로 필터링 (본인 주문만)
    - [x] 최신 주문순 정렬 (`created_at` 내림차순)
    - [x] 주문 상태별 필터링 (선택사항)
  - [x] 주문 목록 UI
    - [x] 주문 카드 컴포넌트 (`components/order-card.tsx`)
      - [x] 주문 번호, 날짜, 상태, 총액 표시
      - [x] 결제 정보 표시 (결제 수단, 결제일)
      - [x] 주문 상세 페이지로 이동하는 링크
      - [x] 반응형 디자인
      - [x] 호버 효과
    - [x] 주문 목록 컴포넌트 (`components/order-list.tsx`)
      - [x] 주문 카드 목록 표시
      - [x] 주문 상태별 필터링 UI
      - [x] 빈 상태 처리
      - [x] 로딩 상태 표시
      - [x] 필터 변경 시 동적 업데이트
- [x] 주문 상세 보기
  - [x] 주문 상세 페이지 (`/orders/[id]`) - Phase 3에서 이미 구현 완료
    - [x] 동적 라우트 생성 (`app/orders/[id]/page.tsx`)
    - [x] 주문 상세 조회 Server Action (`getOrderById()`)
      - [x] `orders` 테이블 조회
      - [x] `order_items` 테이블 조회 (JOIN)
    - [x] 주문 정보 표시
      - [x] 주문 번호, 날짜, 상태
